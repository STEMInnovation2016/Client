    ...
    QObject(parent), mTcpSocket(0), mIpAddress(ipAddress), mPort(port)
{
    ...
    for (int i = 0; i < 256; ++i) {
        if (i < threshold)
            colors[i] = QColor::fromHsl(0, 0, 255*i/threshold).rgb();
        else
            colors[i] = QColor::fromHsl(255*(i-threshold)/(255-threshold), 255, 128).rgb();
    }
...
void Client::readSlot()
{
    ...
    QByteArray newRead = mTcpSocket->readAll();
    read.append(newRead);

    int pos = read.indexOf(QByteArray(2, 0));
    //qDebug() << pos << read.size();
    if (pos==-1 || read.size()-pos < blockSize) { hold = false; return; }
    //qDebug() << "!";

    unsigned short *data = (unsigned short *)(read.data() + pos);
    ...
    for (r = 0; r < 60; ++r) {
        qint32 *row = (qint32 *) image.scanLine(r);
        for (c = 0; c < 80; ++c)
            row[c] = colors[(uchar) ((data[80*r+c] - minValue)*256 / (maxValue-minValue+1))];
    }
